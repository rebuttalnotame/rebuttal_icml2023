...
def run():
    ...
    for i in range(len(sample_sizes)):
        dir_ms = "../data/bodyfat/" + str(names[i]) + "/ms/"
        dir_cv = "../data/bodyfat/" + str(names[i]) + "/cv/"
        ...
        cov_ss, cpl_ss, r2_ss = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=True, ord=14)
        cov_sf, cpl_sf, r2_sf = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=False, ord=14)
    ...
    stat_path = "../results/bodyfat/stat_bodyfat_50_100_200.npz"
    ...
    ylim = (0, 3)
    xlabel, ylabel, fname = "Sample size", "CI length", "../results/bodyfat/figures/cpl_bodyfat_50_100_200.pdf"
    util_interactions.plot(data_mlp, data_rf, data_lf, data_ls, data_sf, data_ss, sample_sizes, xlabel, ylabel, fname,
                           ylim)

    ylim = (0.5, +1.2)
    xlabel, ylabel, fname = "Sample size", "r2 score", "../results/bodyfat/figures/r2_bodyfat_50_100_200.pdf"
