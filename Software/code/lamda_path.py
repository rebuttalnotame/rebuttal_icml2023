...
def explore(X, y, beta, nu, A, lmd, alpha=0.0, max_depth=None, search=0):
    p = X.shape[1]
    Q = Queue(maxsize=0)
...
def initial_Search(X, y, beta, nu, A, lmd=None, max_depth=None):
    d_opt, j, counts, ts = explore(X, y, beta, nu, A, lmd, max_depth=max_depth, search=0)
    return d_opt, j, counts, ts


def main_Search(X, y, beta, nu, A, lmd, alpha=0.0, max_depth=None):
    d_opt, j, counts, ts = explore(X, y, beta, nu, A, lmd, alpha=alpha, max_depth=max_depth, search=1)
    return d_opt, j, counts, ts


def sort_A(A):
    A_flat = [a[0] for a in A]
    return np.sort(A_flat)


def run_selection_path(X, y, lmd_tgt=None, alpha=0.0, max_depth=None, verbose=0):
    beta = np.zeros(1)
    nu = np.zeros(1)
    ...
    while lmd > 0.:
        ...
        d1, j_in, counts, ts = main_Search(X, y, beta, nu, A, lmd, alpha=alpha, max_depth=max_depth)  # Step-size inclusion
