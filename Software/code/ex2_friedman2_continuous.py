...
def plot(data_mlp, data_rf, data_lf, data_ls, data_sf, data_ss, n_samples, xlabel, ylabel, fname, ylim,
    ...
    # pdb.set_trace()
    bp_mlp = plt.boxplot(data_mlp, patch_artist=True, positions=[3], labels=no_labels,
                         boxprops=dict(facecolor="C0", alpha=0.5))

    bp_rf = plt.boxplot(data_rf, patch_artist=True, positions=[4], labels=no_labels,
                        boxprops=dict(facecolor="C1", alpha=0.5))

    bp_ls = plt.boxplot(data_ls, patch_artist=True, positions=[5], labels=n_samples,
                        boxprops=dict(facecolor="C2", alpha=0.5))

    bp_lf = plt.boxplot(data_lf, patch_artist=True, positions=[6], labels=no_labels,
                        boxprops=dict(facecolor="C3", alpha=0.5))

    bp_ss = plt.boxplot(data_ss, patch_artist=True, positions=[7], labels=no_labels,
                        boxprops=dict(facecolor="C4", alpha=0.5))

    bp_sf = plt.boxplot(data_sf, patch_artist=True, positions=[8], labels=no_labels,
...
def run(data_path="../data/friedman2_continuous/"):
    sample_sizes = [500]
    names = ['ss4']
    ...
    for i in range(len(sample_sizes)):
        dir_ms = data_path + str(names[i]) + "/ms/"
        ...
        #
        cov_ss, cpl_ss, r2_ss = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=True, ord=4)
        cov_sf, cpl_sf, r2_sf = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=False, ord=4)
    ...
    stat_path = "../results/friedman2_continuous/stat_friedman2_continuous_500.npz"
    ...
    ylim = (0, +1.5)
    xlabel, ylabel, fname = "Sample size", "r2 score", "../results/friedman2_continuous/figures" \
                                                       "/r2_friedman2_continuous_500.pdf"
    plot(r2_mlps, r2_rfs, r2_lfs, r2_lss, r2_sfs, r2_sss, sample_sizes, xlabel, ylabel, fname, ylim)

    ylim = (0, +3.5)
    xlabel, ylabel, fname = "Sample size", "CI length", "../results/friedman2_continuous/figures" \
                                                        "/cpl_friedman2_continuous_500.pdf"
    plot(data_mlp, data_rf, data_lf, data_ls, data_sf, data_ss, sample_sizes, xlabel, ylabel, fname, ylim)


def friedman2_continuous():
    run(data_path="../data/friedman2_continuous/")


if __name__ == "__main__":
    friedman2_continuous()
